const RecipeReadme = () => (
    <article className="mt-5" aria-details="Recipe Project Readme">
        ...
        <ul>
            ...
            <li>The AppRouter includes React lazy loading pages, public routes, private routes for only users who are logged in. If they're not logged in, it'll redirect them to the homepage if they attempt to load a user page.</li>
            ...
            <li>React-redux-loading-bar is used throughout to display a loading of information. There's also a loading message while going between routes that are being downloaded.</li>
        </ul>
        ...
        <ul>
            <li>The EditRecipePage and SubmitRecipePage both use the same RecipeForm component. If it's a new recipe, information will be cleared and a user can fill in the information. If a user is editing a recipe, the recipe information will be filled in so they can see the current information and modify it.</li>
            <li>RecipeForm includes error handling in case something isn't filled out or profanity is detected. A user can add and delete ingredients with ease using state changing.</li>
        ...
        <ul>
            ...
            <li>
                <ul>
                    <li>Process usernames retrieves usernames per recipe and stores them in the store. To save network traffic, it won't re-fetch if we already have a match between a username and a recipe owner's id. It will simply reuse the same username if the id matches what we've already fetched. If the account no longer exists, it'll store Account Not Found as the username.</li>
                </ul>
            </li>
        </ul>
        ...
        <ul>
            ...
            <li>They include actions for the redux store that'll be dispatched based on which one is called. The files send all data from the server to the appropriate actions and reducers, including errors.</li>
        ...
        <ul>
            ...
            <li>In the base folder, there is _base.scss that includes overall base settings and universally used styling. There's _mixins.scss that is used for pre-defined styles that tend to be repeats. There's also _settings.scss which includes SCSS variables to use throughout the other files.</li>
        ...
        <ul>
            ...
            <li>
                <ul>
                    <li>The file _populate-recipes.scss utilizes CSS Grid to display recipes. It'll adjust how many columns of recipes will display based on screen size. It is used only used in the PopulateRecipe component, which is used by SearchResultsPage and AllRecipesPage.</li>
                </ul>
            </li>
            <li>
                <ul>
            ...
            <li>
                <ul>
                    <li>The _user-profile-page.scss is for the center column of the profile and uses CSS Grid to display recipes. It'll adjust based on the resizing of the screen or browser.</li>
                </ul>
            </li>
            <li>
                <ul>
            ...
            <li>
                <ul>
                    <li>When the screen is large enough, it'll use the regular desktop view of the two nav rows which are based on the _nav-bottom.scss, _nav-top.scss, and _nav.scss. When the screen is smaller, the app will use _nav-popout.scss and _nav.scss.</li>
                </ul>
            </li>
        </ul>
        ...
        <ul>
            ...
            <li>The config checks for settings to decide if we're in development and production and will use different methods, depending on what it is.</li>
            <li>For plugins:</li>
        </ul>
        <ul>
            <li>
